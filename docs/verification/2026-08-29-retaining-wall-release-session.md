...
The director selected automatic checks. The account already had one three-day CCG performance-review schedule, so its exact configuration was saved to `/home/ubuntu/ccg_seo_conversion_2026-08-29/ccg-performance-review-schedule-baseline.json` before temporary reuse. The temporary gate checked policy status every three hours and remained fail-closed while review was pending. It prohibited unrelated SEO, negative-keyword, PMax, Tradenet, budget, bidding, geography or conversion-goal changes.

Google later returned final RSA `822744781388` as `REVIEWED / APPROVED`. Immediately before routing, the paid page returned HTTP 200 with `noindex, follow`, the retaining-wall title was present, and the bounded apex/www five-step quote verifier passed on its first attempt. Both exact retaining-wall keywords were then confirmed enabled, preventing a pre-switch delivery gap.

The already validated atomic mutation returned HTTP 200, enabled dedicated ad group `198409264974`, and paused only the original mixed-group criterion `203212070521~309931182429`. Independent readback confirmed the dedicated exact keyword `198409264974~309931182429` remains enabled, final RSA `822744781388` remains enabled and approved, and its only final URL is `https://concreteconceptsgroup.com/lp/retaining-wall-brisbane`.

Post-switch safeguards passed. Search remains A$60/day, Maximise conversion value at 300% target ROAS, Google Search and Search Network enabled, Search Partners disabled, Content Network disabled, and Presence-only positive/negative location modes intact. Search and PMax remain attached to `CCG Quote Form Only`; Quote Form Submission `7546454804` is still the sole primary action, while SMS, Visualiser, Callback, WhatsApp and Guide remain secondary observations. The temporary approval gate then restored the original active three-day `Concrete Concepts Performance Review` schedule exactly.

Rollback remains the validated inverse mutation: pause the dedicated ad group and re-enable original criterion `203212070521~309931182429`. No budget, PMax asset group, conversion goal, geography or unrelated keyword changed as part of the routing switch.
